...
class StudentPage:
    def __init__(self,parent_window):
        ...
        label.window = tk.Label(self.window,text='学生登录',bg='green',font=tkFont.Font('Verdana',20),width=30,height=2)
        label.pack()
        ...
        self.student_pass=tk.Entry(self.window,width=30,font=tkFont.Font(size=14),bg='Ivory')
    ...
    def back(self):
        StarPage(self.window)  #显示主窗口 销毁本窗口
...
class AdminMange:
    def __init__(self,parent_window):
        ...
        self.top.title=Label(self.frame_left_top,text='学生信息：',font=('Verdana',20))
        self.top.title.grid(row=0,column=0,columnspan=2, sticky=NSEW,padx=50, pady=10)
        ...
        self.right_top_name_label=Lbael(self.frame_left_top,text='姓名：',font=('Verdana',15))
        ...
        #性别
        self.right_top_gender_label=Lbael(self.frame_left_top,text='性别：',font=('Verdana',15))
        ...
        #年龄
        self.right_top_age_label=Lbael(self.frame_left_top,text='年龄：',font=('Verdana',15))
        ...
        self.rigth_top_button3=ttk.Buttoon(self.frame_right_top,text='删除选中学生信息',width=20,command=self.delete_row)
    ...
    def new_row(self):
        ...
        if self.var_id.get() != '' and self.var_name.get() != '' and self.var_gender.get() != '' and self.var_age.get() != '':
				# 打开数据库连接
            pymysql.connect("localhost", "root", "root", "student")
            cursor = db.cursor()  # 使用cursor()方法获取操作游标
